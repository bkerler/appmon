...
/__/     \__\ | _|      | _|      |__|  |__|  \______/  |__| \__| 
            github.com/dpnishant (Modified by B. Kerler)
...
def init_opts():
    ...
    parser.add_argument('-c', "--clear", action="store_true",
                    help='''Optional; Accepts 1 or 0; Clear all application databases; ''')
    parser.add_argument('-v', action='version', version='AppMon Sniffer v0.1, modified by B.Kerler, 2017')
    ...
    if (results.clear):
        if os.path.exists("app_dumps"):
            shutil.rmtree("app_dumps")
...
def on_message(message, data):
    current_time = time.strftime('%b %d %Y %H:%M %p', time.localtime())
    global output_dir
